...
module.exports = async function (client, interaction) {
    ...
    collector.on('collect', m => {
        ...
        if (m.content.toUpperCase() === captcha.value) {
            ...
            if (tries === 0) {
                ...
                interaction.followUp({content: `:x: Raté ! Il vous reste ${tries} essais`, ephemeral: true})
            }
        }
    });
    collector.on('end', async collected => {
        if (verified === true) {
            ...
            interaction.followUp({content: `:white_check_mark: Bienvenue sur le serveur !`, ephemeral: true})
            ...
            interaction.member.dmChannel.send({content: ":x: Vous avez été expulsé du serveur car vous avez raté vos 3 essais de CAPTCHA.", ephemeral: true})
