...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setName("nb")
        .setDescription("Le nombre de messages à supprimer.").setRequired(true)),
    async execute(client, interaction) {
        const usender = interaction.user;
        const msender = interaction.member;
        const nb = interaction.options.getInteger("nb");
        if (nb % 1 != 0) {
            return interaction.reply({content: "Vous devez entrer un nombre entier !", ephemeral: true})
        }
        if (nb < 1 || nb > 99) {
        ...
        return interaction.reply({content: `Vous avez supprimé avec succès ${nb} messages !`, ephemeral: true})
    },
};