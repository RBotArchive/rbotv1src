...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('userinfo')
		.setDescription('Obtenir vos informations ou celles d\'un autre utilisateur.')
		.addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur dont les informations doivent être montrées')),
		...
		if (user !== null) {
			...
			replyembed = new EmbedBuilder()
				...
				.addFields(
					...
					{name: "Compte créé le :", value: "<t:" + Math.floor(user.createdTimestamp / 1e3) + ":f>"},
					{name: "Status", value: memberstatus},
					{name: "Avatar", value: user.avatarURL()},
					{name: "Tag", value: user.discriminator},
					{name: "ID", value: user.id},
					{name: "A rejoint le :", value: "<t:" + Math.floor(member.joinedTimestamp / 1e3) + ":f>"}
				)
			...
			replyembed = new EmbedBuilder()
				...
				.addFields(
					...
					{name: "Compte créé le :", value: "<t:" + Math.floor(usender.createdTimestamp / 1e3) + ":f>"},
					{name: "Status", value: memberstatus},
					{name: "Avatar", value: usender.avatarURL()},
					{name: "Tag", value: usender.discriminator},
					{name: "ID", value: usender.id},
					{name: "A rejoint le :", value: "<t:" + Math.floor(msender.joinedTimestamp / 1e3) + ":f>"}
				)
