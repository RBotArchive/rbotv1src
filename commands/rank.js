...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("rank")
        .setDescription(
            "Envoie une image du niveau de quelqu'un sur le serveur."
        )
        .addUserOption((option) =>
            option
                .setName("utilisateur")
                .setDescription("L'utilisateur dont le niveau doit être montré")
        ),
    async execute(client, interaction) {
        ...
        currentXp = (await client.xpDB.get(`${user.id}_currentXp`)) | 0;
        currentLevel = (await client.xpDB.get(`${user.id}_currentLevel`)) | 0;
